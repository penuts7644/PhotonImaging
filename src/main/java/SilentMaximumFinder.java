 ...
 * the current image are marked (255; unmarked pixels 0). The plug-in can also create watershed-segmented particles:
...
public class SilentMaximumFinder implements PlugInFilter {
    ...
    private int flags = DOES_ALL | NO_CHANGES | NO_UNDO;// the flags (see interfaces PlugInFilter and ExtendedPlugInFilter)
...
//    private int       nPasses = 0;                  // for progress bar, how many images to process (sequentially or parallel threads)
    ...
    private int intEncodeXMask;               // needed for encoding x and y in a single int (watershed): mask for x
    private int intEncodeYMask;               // needed for encoding x and y in a single int (watershed): mask for y
    private int intEncodeShift;               // needed for encoding x and y in a single int (watershed): shift of y
    /**
    ...
    /**
     * Get estimated "true" height of a maximum or saddle point of a Euclidian Distance Map. This is needed since the
    ...
    /**
     * eliminate unmarked maxima for use by watershed. Starting from each previous maximum, explore the surrounding down
    ...
    /**
     * Delete extra structures form watershed of non-EDM images, e.g., foreground patches, single dots and lines ending
     * somewhere within a segmented particle Needed for post-processing watershed-segmented images that can have local
     * minima
     *
    ...
    /**
     * Do watershed segmentation on a byte image, with the start points (maxima) set to 255 and the background set to 0.
     * The image should not have any local maxima other than the marked ones. Local minima will lead to artifacts that
     * can be removed later. On output, all particles will be set to 255, segmentation lines remain at their old value.
     *
...
//                    IJ.showProgress(1.0);
    ...
    /**
     * Creates the lookup table used by the watershed function for dilating the particles. The algorithm allows dilation
     * in both straight and diagonal directions. There is an entry in the table for each possible 3x3 neighborhood: x-1
     * x x+1 y-1 128 1 2 y 64 pxl_unset_yet 4 y+1 32 16 8 (to find throws entry, sum up the numbers of the neighboring
     * pixels set; e.g. entry 6=2+4 if only the pixels (x,y-1) and (x+1, y-1) are set. A pixel is added on the 1st pass
     * if bit 0 (2^0 = 1) is set, on the 2nd pass if bit 1 (2^1 = 2) is set, etc. pass gives the direction of rotation,
     * with 0 = to top left (x--,y--), 1 to top, and clockwise up to 7 = to the left (x--). E.g. 4 = add on 3rd pass, 3
     * = add on either 1st or 2nd pass.
     */
    ...
    /**
     * create an array of offsets within a pixel array for directions in clockwise order: 0=(x,y-1), 1=(x+1,y-1), ...
     * 7=(x-1,y) Also creates further class variables: width, height, and the following three values needed for storing
     * coordinates in single ints for watershed: intEncodeXMask, intEncodeYMask and intEncodeShift. E.g., for width
     * between 129 and 256, xMask=0xff and yMask = 0xffffff00 are bitwise masks for x and y, respectively, and shift=8
     * is the bit shift to get y from the y-masked value Returns as class variables: the arrays of the offsets to the 8
     * neighboring pixels and the array maskAndShift for watershed
     */

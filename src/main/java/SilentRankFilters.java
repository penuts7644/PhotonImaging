...
/**
 * This plugin implements the Mean, Minimum, Maximum, Variance, Median, Open Maxima, Close Maxima, Remove Outliers,
 * Remove NaNs and Despeckle commands.
 */
...
public class SilentRankFilters implements PlugInFilter {
     ...
     * number of pixels in the kernel area length-1: kernelRadius in x direction (kernel width is 2*kernelRadius+1)
     * Kernel height can be calculated as (array length - 1)/2 (odd number); Kernel radius in y direction is kernel
     * height/2 (truncating integer division). Note that kernel width and height are the same for the circular kernels
     * used here, but treated separately for the case of future extensions with non-circular kernels.
     */
